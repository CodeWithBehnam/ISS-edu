...
export function TelemetryPanel({ telemetry, busy, capability }: TelemetryPanelProps) {
  if (!telemetry) {
    return (
      <div className="rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-lg shadow-sky-100/70 backdrop-blur">
  ...
  return (
    <div className="space-y-4 rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-lg shadow-sky-100/70 backdrop-blur">
      ...
      {hasValidDrift ? (
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs text-amber-700">
          <p className="font-semibold text-amber-600">Drift monitor</p>
      ...
      <div className="rounded-2xl border border-sky-100 bg-sky-50/80 p-4 text-xs text-slate-600">
        <p>Rendering mode: {capability === 'webgl' ? '3D WebGL scene' : '2D canvas fallback'}.</p>
        <p className="mt-1 text-slate-500">Cache status: {cacheStatus}</p>
      </div>
    </div>
  );
}
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 shadow-sm">
      ...
      <dd className="mt-1 font-mono text-base text-slate-900">{value}</dd>
    </div>
  );
}
